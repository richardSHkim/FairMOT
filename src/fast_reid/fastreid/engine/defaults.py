...
from fastreid.data import build_reid_test_loader, build_reid_train_loader, build_reid_st_test_loader, build_camstyle_reid_train_loader
from fastreid.evaluation import (ReidEvaluator, ReidEvaluatorST,
...
from . import hooks
from .train_loop import TrainerBase, AMPTrainer, SimpleTrainer, CamStyleSimpleTrainer, CamStyleAMPTrainer, CamStyleAMPTrainerHalfBatch
...
def default_argument_parser():
    ...
    parser.add_argument("--eval-st-only", action="store_true", help="perform spatial-temporal evaluation only")
...
class DefaultTrainer(TrainerBase):
    ...
    @classmethod
    def build_st_test_loader(cls, cfg, dataset_name):
        """
        Returns:
            iterable
        It now calls :func:`fastreid.data.build_reid_test_loader`.
        Overwrite it if you'd like a different data loader.
        """
        return build_reid_st_test_loader(cfg, dataset_name)
    ...
    @classmethod
    def build_st_evaluator(cls, cfg, dataset_name, output_dir=None):
        data_loader, num_query = cls.build_st_test_loader(cfg, dataset_name)
        return data_loader, ReidEvaluatorST(cfg, num_query, output_dir)
    ...
    def test_st(cls, cfg, model):
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TESTS):
            logger.info("Prepare testing set")
            try:
                data_loader, evaluator = cls.build_st_evaluator(cfg, dataset_name)
            except NotImplementedError:
                logger.warn(
                    "No evaluator found. implement its `build_evaluator` method."
                )
...
class CamStyleDefaultTrainer(DefaultTrainer):
    def __init__(self, cfg):
        ...
        # Assume these objects must be constructed in this order.
        data_loader, data_loader_camstyle = self.build_train_loader(cfg)
        cfg = self.auto_scale_hyperparams(cfg, data_loader.dataset.num_classes)
        model = self.build_model(cfg)
        optimizer = self.build_optimizer(cfg, model)
        ...
        self._trainer = (CamStyleAMPTrainer if cfg.SOLVER.FP16_ENABLED else CamStyleSimpleTrainer)(
            model, data_loader, data_loader_camstyle, optimizer
        )
    ...
    def build_train_loader(cls, cfg):
        ...
        logger = logging.getLogger(__name__)
        logger.info("Prepare training set")
        return build_camstyle_reid_train_loader(cfg)


class CamStyleDefaultTrainerHalfBatch(DefaultTrainer):
    def __init__(self, cfg):
        TrainerBase.__init__(self)
        ...
        self._trainer = (CamStyleAMPTrainerHalfBatch if cfg.SOLVER.FP16_ENABLED else CamStyleSimpleTrainer)(
            model, data_loader, data_loader_camstyle, optimizer
        )
