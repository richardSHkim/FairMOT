...
class CamStyleSimpleTrainer(TrainerBase):
    """
    A simple trainer for the most common type of task:
    single-cost single-optimizer single-data-source iterative optimization.
    It assumes that every step, you:
    ...
    def __init__(self, model, data_loader, camstyle_loader, optimizer):
        """
        Args:
            model: a torch Module. Takes a data from data_loader and returns a
                dict of heads.
        ...
        self._data_loader_iter = iter(data_loader)
        self._camstyle_loader_iter = iter(camstyle_loader)
        self.optimizer = optimizer
    ...
    def run_step(self):
        ...
        """
        data = next(self._data_loader_iter)
        camstyle_data = next(self._camstyle_loader_iter)
        data_time = time.perf_counter() - start
        ...
        camstyle_loss_dict = self.model(camstyle_data)
        losses += sum(camstyle_loss_dict.values())
...
class CamStyleAMPTrainer(CamStyleSimpleTrainer):
    """
    Like :class:`SimpleTrainer`, but uses apex automatic mixed precision
    in the training loop.
    """
    ...
    def run_step(self):
        ...
        loss_dict = self.model(data)
        loss_dict_camstyle = self.model.forwarde_camstyle(camstyle_data)
        losses = sum(loss_dict.values()) + sum(loss_dict_camstyle.values())
...
class CamStyleAMPTrainerHalfBatch(CamStyleSimpleTrainer):
    """
    Like :class:`SimpleTrainer`, but uses apex automatic mixed precision
    in the training loop.
    """
    ...
    def run_step(self):
        ...
        for k in data.keys():
            if k=='img_paths':
                data_[k] = data[k] + camstyle_data[k]
            else:
                data_[k] = torch.cat([data[k], camstyle_data[k]], dim=0)

        loss_dict = self.model(data_)
        losses = sum(loss_dict.values())
