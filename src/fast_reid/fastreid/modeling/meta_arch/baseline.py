...
class Baseline(nn.Module):
    ...
    def forwarde_camstyle(self, batched_inputs):
        images = self.preprocess_image(batched_inputs)
        features = self.backbone(images)
        ...
        if self.training:
            ...
            outputs = self.heads(features, targets)
            losses = self.losses_camstyle(outputs, targets)
            return losses
        else:
            outputs = self.heads(features)
            return outputs
    ...
    def losses_camstyle(self, outputs, gt_labels):
        """
        Compute loss from modeling's outputs, the loss function input arguments
        must be the same as the outputs of the model forwarding.
        """
        ...
        if "CrossEntropyLoss" in loss_names:
            loss_dict["loss_cls"] = cross_entropy_loss(
                ...
                self._cfg.MODEL.LOSSES.CE.EPSILON,
                self._cfg.MODEL.LOSSES.CE.ALPHA,
            ) * self._cfg.MODEL.LOSSES.CE.CAMSTYLE_SCALE
