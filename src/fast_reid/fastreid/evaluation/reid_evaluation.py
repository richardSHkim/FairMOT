...
from .rerank import re_ranking2
...
class ReidEvaluatorST(DatasetEvaluator):
    def __init__(self, cfg, num_query, output_dir=None):
        self.cfg = cfg
        self._num_query = num_query
        self._output_dir = output_dir
    ...
    def process(self, inputs, outputs):
        self.pids.extend(inputs["targets"])
        self.camids.extend(inputs["camids"])
        self.frames.extend(inputs["frames"])
        self.features.append(outputs.cpu())
    ...
    def evaluate(self):
        ...
        query_pids = np.asarray(pids[:self._num_query])
        query_camids = np.asarray(camids[:self._num_query])
        query_frames = np.asarray(frames[:self._num_query])
        ...
        # assert False, score.shape # [num_queris, num_gallery]

        # SPATIAL TEMPORAL SCORE ####################################
        result2 = scipy.io.loadmat(os.path.join(self.cfg.OUTPUT_DIR, '..', 'pytorch_result2.mat'))
        ...
        if self.cfg.TEST.RERANK.ENABLED:
            ...
            all_scores = st_evaluation(features, pids, camids, frames,
                                features, pids, camids, frames,
                                score,
                                alpha=self.cfg.TEST.SPATIALTEMPORAL.ALPHA, smooth=self.cfg.TEST.SPATIALTEMPORAL.SMOOTH, 
                                distribution=distribution)

            rerank_dist = re_ranking2(self._num_query, all_scores)
