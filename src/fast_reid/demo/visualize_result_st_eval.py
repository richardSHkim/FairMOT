...
if __name__ == '__main__':
    ...
    camids = []
    frames = []
    for (feat, pid, camid, frame) in tqdm.tqdm(demo.run_on_st_loader(test_loader), total=len(test_loader)):
        feats.append(feat)
        pids.extend(pid)
        camids.extend(camid)
        frames.extend(frame)
    ...
    score = st_evaluation(q_feat, q_pids, q_camids, q_frames, 
                g_feat, g_pids, g_camids, g_frames,
                score, 
                alpha=cfg.TEST.SPATIALTEMPORAL.ALPHA, smooth=cfg.TEST.SPATIALTEMPORAL.SMOOTH, distribution=distribution)
    distmat = 1 - score
    #############################################################
    ...
    if cfg.TEST.RERANK.ENABLED:
        ...
        features_norm = F.normalize(feats, p=2, dim=1)
