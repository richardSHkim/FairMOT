...
class FeatureExtractionDemo(object):
    ...
    def run_on_st_loader(self, data_loader):
        if self.parallel:
            buffer_size = self.predictor.default_buffer_size
            ...
            for cnt, batch in enumerate(data_loader):
                ...
                if cnt >= buffer_size:
                    batch = batch_data.popleft()
                    predictions = self.predictor.get()
                    yield predictions, batch["targets"].numpy(), batch["camids"].numpy(), batch["frames"].numpy()
    ...
    def run_on_vid_loader(self, data_loader):
        if self.parallel:
            buffer_size = self.predictor.default_buffer_size
            ...
            for cnt, batch in enumerate(data_loader):
                ...
                if cnt >= buffer_size:
                    batch = batch_data.popleft()
                    predictions = self.predictor.get()
                    yield predictions, batch["targets"].numpy(), batch["camids"].numpy(), batch["img_paths"]
